...
class ControllerExtensionPaymentCointopayCC extends Controller
{
    public function index()
    {
        ...
        if (isset($this->session->data['order_id'])) {
            ...
            $data = array();
            $this->load->language('extension/payment/cointopay_cc_invoice');
            $this->load->model('checkout/order');
            $data['text_failed'] = $order_info;
            $data['footer'] = $this->load->controller('common/footer');
            $data['header'] = $this->load->controller('common/header');
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'template/extension/payment/cointopay_cc_failed')) {
                return $this->response->setOutput($this->load->view('extension/payment/cointopay_cc_failed', $data));
            } else {
                return $this->response->setOutput($this->load->view('extension/payment/cointopay_cc_failed', $data));
            }
        }
        try {
            ...
            if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
                if (empty($this->config->get('payment_cointopay_cc_merchantID')) || empty($this->config->get('payment_cointopay_cc_securitycode'))) {
                    echo 'CredentialsMissing';
                    exit();
                }
                $formData = $this->request->post;
                $currencyOutput = $this->getInputCurrencyList();
                if (null !== $this->config->get('payment_cointopay_cc_merchantID')) {
                ...
                if ($php_arr->error == '' || empty($php_arr->error)) {
                    $this->model_checkout_order->addOrderHistory($php_arr->CustomerReferenceNr, $this->config->get('payment_cointopay_cc_order_status_id'), 'Transaction #' . $php_arr->TransactionID . ' Created on cointopay for OrderID #' . $php_arr->CustomerReferenceNr . ' with status Processing', false);
                    ...
                    $data1['text_cc_payment_detail'] = $this->language->get('text_cc_payment_detail');
            ...
            $htmlDom->loadHTML($php_arr->PaymentDetailCConly);
            ...
            if (!empty($matches)) {
				if ($matches[0] != '') {
					$this->response->redirect($matches[0]);
				} else {
					$data1['error'] = 'Payment link is empty';
				}
            } else {
                $data1['error'] = 'pattern not match';
			}
                $data1['footer'] = $this->load->controller('common/footer');
                $data1['header'] = $this->load->controller('common/header');
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/cointopay_cc_invoice')) {
                    $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/payment/cointopay_cc_invoice', $data1));
                } else {
                    $this->response->setOutput($this->load->view('extension/payment/cointopay_cc_invoice', $data1));
                }
            } else {
                $this->load->language('extension/payment/cointopay_cc');

                $data['action'] = $this->url->link('extension/payment/cointopay_cc', '', true);

                $data['price'] = $this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false);
                $data['key'] = $this->config->get('payment_cointopay_cc_api_key');
                $data['AltCoinID'] = $this->config->get('payment_cointopay_cc_crypto_coin');
                $data['crypto_coins'] = $this->getMerchantCoins($this->config->get('payment_cointopay_cc_merchantID'));
                $data['OrderID'] = $this->session->data['order_id'];
                $data['currency'] = $order_info['currency_code'];

                $data['text_crypto_coin_label'] = $this->language->get('text_crypto_coin_label');
               
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/cointopay_cc')) {
                    return $this->load->view($this->config->get('config_template') . '/template/extension/payment/cointopay_cc', $data);
                } else {
                    return $this->load->view('extension/payment/cointopay_cc', $data);
                }
            }
    ...
    {
        $merchantId = $this->config->get('payment_cointopay_cc_merchantID');
    ...
    {
        $merchantid = $this->config->get('payment_cointopay_cc_merchantID');
        $payment_cointopay_securitycode = $this->config->get('payment_cointopay_cc_securitycode');
        return $this->c2pCurl('SecurityCode=' . $payment_cointopay_securitycode . '&MerchantID=' . $merchantid . '&Amount=' . $data['price'] . '&AltCoinID=' . $data['AltCoinID'] . '&inputCurrency=' . $data['currency'] . '&output=json&CustomerReferenceNr=' . $data['OrderID'] . '&returnurl=' . $this->url->link('extension/payment/cointopay_cc/callback') . '&transactionconfirmurl=' . $this->url->link('extension/payment/cointopay_cc/callback') . '&transactionfailurl=' . $this->url->link('extension/payment/cointopay_cc/callback'), $data['key']);
    }
    ...
    public function c2pCurl($data, $apiKey, $post = false)
    {
        ...
        if ($post) {
            $formData = $post;
            $formData['transactionconfirmurl'] = $this->url->link('extension/payment/cointopay_cc/callback');
            $formData['transactionfailurl'] = $this->url->link('extension/payment/cointopay_cc/callback');
    ...
    public function callback()
    {
        ...
        if (isset($_REQUEST['status'])) {
            ...
            $data = [
                'mid' => $this->config->get('payment_cointopay_cc_merchantID'),
            ...
            if (200 !== $transactionData['status_code']) {
                ...
                if ($transactionData['data']['Security'] != $_REQUEST['ConfirmCode']) {
                    ...
                    if ($_REQUEST['status'] == 'paid' && $_REQUEST['notenough'] == 0) {
                        ...
                        $this->model_checkout_order->addOrderHistory($_REQUEST['CustomerReferenceNr'], $this->config->get('payment_cointopay_cc_callback_success_order_status_id', 'Successfully Paid'), 'Cointopay Transaction #' . $_REQUEST['TransactionID'] . ' Set to completed for OrderID #' . $_REQUEST['CustomerReferenceNr'], false);
                        $data['text_success'] = $this->language->get('text_success');
                        $data['footer'] = $this->load->controller('common/footer');
                        $data['header'] = $this->load->controller('common/header');

                        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/cointopay_cc_success')) {
                            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/payment/cointopay_cc_success', $data));
                        } else {
                            $this->response->setOutput($this->load->view('extension/payment/cointopay_cc_success', $data));
                        }
                    } elseif ($_REQUEST['status'] == 'paid' && $_REQUEST['notenough'] == 1) {
                        $this->model_checkout_order->addOrderHistory($_REQUEST['CustomerReferenceNr'], $this->config->get('payment_cointopay_cc_callback_notenough_order_status_id'), 'Cointopay Transaction #' . $_REQUEST['TransactionID'] . ' Set to pending (underpaid) for OrderID #' . $_REQUEST['CustomerReferenceNr'], false);
                        ...
                        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/cointopay_cc_failed')) {
                            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/payment/cointopay_cc_failed', $data));
                        } else {
                            $this->response->setOutput($this->load->view('extension/payment/cointopay_cc_failed', $data));
                        }
                    } elseif ($_REQUEST['status'] == 'failed') {
                        $this->model_checkout_order->addOrderHistory($_REQUEST['CustomerReferenceNr'], $this->config->get('payment_cointopay_cc_callback_failed_order_status_id', 'Transaction payment failed'), 'Cointopay Transaction #' . $_REQUEST['TransactionID'] . ' Set to failed for OrderID #' . $_REQUEST['CustomerReferenceNr'], false);
                        ...
                        $this->model_checkout_order->addOrderHistory($_REQUEST['CustomerReferenceNr'], $this->config->get('payment_cointopay_cc_callback_expired_order_status_id'), 'Cointopay Transaction #' . $_REQUEST['TransactionID'] . ' Set to expired for OrderID #' . $_REQUEST['CustomerReferenceNr'], false);
                        ...
                        $this->model_checkout_order->addOrderHistory($_REQUEST['CustomerReferenceNr'], $this->config->get('payment_cointopay_cc_callback_cancel_order_status_id'), 'Cointopay Transaction #' . $_REQUEST['TransactionID'] . ' Set to ' . $_REQUEST['status'] . ' for OrderID #' . $_REQUEST['CustomerReferenceNr'], false);
                        ...
                        $this->model_checkout_order->addOrderHistory($_REQUEST['CustomerReferenceNr'], $this->config->get('payment_cointopay_cc_callback_cancel_order_status_id'), 'Cointopay Transaction #' . $_REQUEST['TransactionID'] . ' Set to canceled for OrderID #' . $_REQUEST['CustomerReferenceNr'], false);
    ...
    public function getCoinsPaymentUrl()
    {
        ...
        if (isset($_REQUEST['TransactionID'])) {
            $url = 'https://app.cointopay.com/CloneMasterTransaction?MerchantID=' . $this->config->get("payment_cointopay_cc_merchantID") . '&TransactionID=' . $_REQUEST["TransactionID"] . '&output=json';
            $ch = curl_init($url);
